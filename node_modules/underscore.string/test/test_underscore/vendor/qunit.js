...
var QUnit = {
	...
	asyncTest: function asyncTest(test1Name, expected, callback) {
		if ( arguments.length === 2 ) {
			callback = expected;
			expected = 0;
		}

		QUnit.test(test1Name, expected, callback, true);
	},

	test: function test(test1Name, expected, callback, async) {
		var name = test1Name, testEnvironment = {};
		...
		synchronize(function() {
			...
			QUnit.testDone( test1Name, bad, config.assertions.length );
...
var Test = function(name, test1Name, expected, testEnvironmentArg, async, callback) {
	this.name = name;
...
Test.prototype = {
	...
	setup: function() {
		...
		QUnit.testStart( this.test1Name, this.testEnvironment );
		...
		try {
			...
			// TODO use test1Name instead of name for no-markup message?
	...
	finish: function() {
		...
		if ( tests ) {
			...
			defined.sessionStorage && sessionStorage.setItem("qunit-" + this.test1Name, bad);
		...
		QUnit.testDone( this.test1Name, bad, this.assertions.length );
	},
	...
	queue: function() {
		...
		var bad = defined.sessionStorage && +sessionStorage.getItem("qunit-" + this.test1Name);
		if (bad) {
			run();
		} else {
			synchronize(run);
...
var QUnit = {
	...
	asyncTest: function(test1Name, expected, callback) {
		if ( arguments.length === 2 ) {
			callback = expected;
			expected = 0;
		}
	...
	test: function(test1Name, expected, callback, async) {
		var name = '<span class="test-name">' + test1Name + '</span>', testEnvironmentArg;
		...
		if ( !validTest(config.currentModule + ": " + test1Name) ) {
			return;
		}

		var test = new Test(name, test1Name, expected, testEnvironmentArg, async, callback);
	...
	if ( banner ) {
		...
		if ( paramsIndex > -1 ) {
			...
			if ( mainPageLocation == location.href ) {
				...
				var test1Name = decodeURIComponent(location.search.slice(1));
				banner.innerHTML = '<a href="' + mainPageLocation + '">' + banner.innerHTML + '</a> &#8250; <a href="">' + test1Name + '</a>';
			}
		}
	}
